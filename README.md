# Aaron Cottrell

Head of Product at [Validation Cloud](https://validationcloud.io).
10+ years building products across Web3, blockchain, and enterprise B2B SaaS.
Previously Zendesk, Elliptic, Vault Platform, PeerIndex. Based in London.

---

## Projects

**[Tailwise](https://github.com/acottrell/tailwise)** - Wind-optimised cycling route planner.
Ranks routes by tailwind advantage on the way home using live weather data.
Next.js, Mapbox, Strava, Open-Meteo. [Live site](https://tailwise-cycle.vercel.app/)

---

## Contributions

**[Dex](https://github.com/davekilleen/Dex)** — [PR #4](https://github.com/davekilleen/Dex/pull/4):
Google Calendar sync via macOS EventKit, with two-step setup docs. Python, 260+ stars.

---

## Content

**[Around the Block](https://open.spotify.com/show/1CBPjUjBYrDdRZXwxBvbdc)** — Podcast series interviewing
builders, developers, and product leaders across the Web3 ecosystem.

---

## Interests

The craft of product leadership. How AI changes what it means to build software.

---

[LinkedIn](https://www.linkedin.com/in/aaroncottrell/) · [acottrell.com](https://acottrell.com)

